...
const DeleteGatewayEmailStep = ({ email, onVerified }) => {
  ...
  const startPolling = () => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const result = await pollEmailVerification(email, 6);
  ...
  useEffect(() => {
    ...
    if (!hasSentRef.current) {
      ...
      deleteGatewayEmailAPI(email).catch((err) =>
        console.error("Error sending delete gateway email:", err),
      );
    }
    startPolling();
    return () => stopPolling();
  ...
  const handleResendEmail = async () => {
    ...
    setIsSending(true);
    try {
      const result = await deleteGatewayEmailAPI(email);
      if (result?.status === 200) {
        Toast.success(
...
export default DeleteGatewayEmailStep;
