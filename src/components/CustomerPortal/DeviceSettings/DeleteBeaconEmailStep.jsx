...
const DeleteBeaconEmailStep = ({ email, onVerified }) => {
  ...
  const startPolling = () => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const result = await pollEmailVerification(email, 7);
  ...
  useEffect(() => {
    ...
    if (!hasSentRef.current) {
      ...
      deleteBeaconEmailAPI(email).catch((err) =>
        console.error("Error sending delete beacon email:", err),
      );
    }
    startPolling();
    return () => stopPolling();
  ...
  const handleResendEmail = async () => {
    ...
    setIsSending(true);
    try {
      const result = await deleteBeaconEmailAPI(email);
      if (result?.status === 200) {
        Toast.success(
...
export default DeleteBeaconEmailStep;
