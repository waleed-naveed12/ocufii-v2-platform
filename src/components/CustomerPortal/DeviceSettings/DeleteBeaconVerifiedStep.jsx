...
const DeleteBeaconVerifiedStep = ({ email, mac }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      const result = await deleteBeaconDirectAPI({ email, mac });
      if (result?.status === 204 || result?.status === 200) {
        Toast.success(result?.message || "Beacon deleted successfully.");
      } else {
        Toast.error(result?.message || "Failed to delete beacon.");
      }
    } catch {
      ...
      <EmailContainer>
        <EmailCard>
          ...
          <VerifiedText>
            ...
            delete this beacon from your account.
          </VerifiedText>
...
export default DeleteBeaconVerifiedStep;
